...
const STORAGE_FILE_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(jpe?g|png|webp|gif)$/i

/** Storage object key: `{profileUuid}/{randomUuid}.{ext}` */
function normalizeAndValidatePath(raw: string): string | null {
  try {
    ...
    if (!PROFILE_ID_RE.test(profileId) || !STORAGE_FILE_RE.test(fileName)) return null
...
/** Same-origin proxy so images load even when the bucket is private. */
